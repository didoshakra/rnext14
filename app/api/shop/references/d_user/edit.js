///pages/api/d_user/product/edit.js

import { pool } from "../../../../../config/dbShop"

export default function handler(req, resp) {
  const data = JSON.parse(req.body) //Для feth- не треба переиворення json ->obj
  const sql = `UPDATE d_user SET name = '${data.name}',last_name = '${data.last_name}',login = '${data.login}',password = '${data.password}',profile = '${data.profile}' WHERE id IN (${data.id})`
