...
import { userSchema } from "./brandSchema"
...
export default function UserForm() {
  ...
  return (
    <div className="bg-eclipseBg absolute inset-0 z-20 mx-2 max-w-full">
      <form
        className="absolute right-0 bottom-1 z-10  rounded-xl border border-fBorder bg-fBg1 dark:border-fBorderD dark:bg-fBg1D md:left-auto  p-2  mx-auto"
      ...
      >
        ...
        <div className="mt-5">
          <label>Name</label>
          <input type="text" placeholder="Enter your name" className={inputClass} {...register("name")} />
        ...
        <div className="mt-5">
          ...
          />
          <span className="text-red-500 font-bold">{errors.password_confirmation?.message}</span>
        </div>
        {/* <div className="mt-5">
          ...
          <span className="text-red-500 font-bold">{errors.hobbies?.message}</span>
        </div> */}
        <div className="mt-5">
          <label>Profile Image</label>
          <input type="file" onChange={handleImage} className={inputClass} />
          <span className="text-red-500 font-bold">{errors.profile?.message}</span>
        </div>
        <div className="mt-5">
          <button className="bg-red-500 w-full p-2 h-10 rounded-lg">Submit</button>
        </div>
      </form>
    </div>
  )
