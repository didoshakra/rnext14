...
export async function addClient(message, formData) {
  const data = formData
    console.log("action_pg/data=", data)
  ...
  const sql =
    "insert into d_client(name,last_name,email,skod,discount_proc) VALUES(${data.name},${data.last_name},${data.email},${data.skod},${discount_proc}) RETURNING *"
  //   const sqlvalues = [data.name, data.last_name, data.email, data.skod, data.discount_proc]
  //   try {
  //--- pool(promise)
